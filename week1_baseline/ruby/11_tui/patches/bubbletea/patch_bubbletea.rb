...
# rebuilds the native extension (.so on Linux, .bundle on macOS). The patch
# makes program_poll_event() keep a pending-bytes
...
# DLEXT is "so" on Linux, "bundle" on macOS (mkmf picks it per-platform).
...
# macOS's (LLVM/BSD) strip has no long-option GNU syntax; -S is its
# "strip debug symbols only" equivalent to GNU's --strip-debug.
strip_debug_flag = RbConfig::CONFIG["host_os"] =~ /darwin/ ? "-S" : "--strip-debug"

puts "Rebuilding extension:"
sh("make clean 2>/dev/null; ruby extconf.rb", chdir: ext_dir)
sh("make", chdir: ext_dir)
sh("strip #{strip_debug_flag} bubbletea.#{dlext}", chdir: ext_dir)
...
# 5. Sanity-check: the rebuilt extension loads and exposes poll_event.
load_ok = system(RbConfig.ruby, "-e",
                 'require "bubbletea"; exit(Bubbletea::Program.instance_methods.include?(:poll_event) ? 0 : 1)')
die "rebuilt extension failed to load / poll_event missing" unless load_ok
